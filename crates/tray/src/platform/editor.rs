...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    label.check { display:flex; align-items:center; gap:9px; color:var(--ink); }
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .layout-canvas { position:relative; height:280px; overflow:hidden; background:linear-gradient(90deg,#f4f6f7 1px,transparent 1px),linear-gradient(#f4f6f7 1px,transparent 1px); background-size:24px 24px; border:1px solid var(--line); }
    .layout-device { position:absolute; width:150px; min-height:68px; padding:10px; transform:translate(-50%,-50%); color:var(--ink); background:white; border:2px solid var(--accent); border-radius:5px; box-shadow:0 5px 16px rgba(0,0,0,.12); cursor:grab; touch-action:none; }
    .layout-device small { display:block; margin-top:5px; color:var(--muted); }
  ...
  <div class="wizard" id="wizard" hidden>
    <div class="wizard-window" role="dialog" aria-modal="true">
      ...
      <div class="wizard-body">
        ...
        <section class="step"><h2>Report this machine to the registry</h2><p class="copy">Only this machine's ID, local IP address and listening TCP port are sent. No peer is queried. The first certificate presented by a selected peer will be stored using trust on first use.</p><div class="grid"><label class="wide">Registry endpoint<input id="wizard_registry" type="url" placeholder="https://registry.example.com"></label><label>Listening TCP port<input id="wizard_port" type="number" min="1" max="65535"></label></div><div class="result" id="register_result">Ready to register this machine.</div></section>
      </div>
