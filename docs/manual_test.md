...
4. 在 A 机文件管理器中复制多个文件，等待日志出现
   `received clipboard files are ready to paste`，然后在 B 机目录中粘贴并逐个校验内容。
5. 在 A 机使用系统截图快捷键把截图放进剪贴板，等待 B 机出现
   `applied clipboard image received from peer`，然后粘贴到支持图片的应用并核对内容；
   反向重复。
6. 传输截图或一个较大文件时持续快速打字；键盘延迟和顺序不应受剪贴板流影响。
7. 只阻断 `clipboard_port`（默认 `5006`）后继续打字：键盘转发必须正常，日志只应
